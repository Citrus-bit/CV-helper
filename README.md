...
面向电脑浏览器的本地简历分析工作台。它优先读取 PDF 原生文字层，只对扫描页或缺失区块调用 OCR；随后建立 Resume AST 与证据关系，提供逐条建议、JD 证据匹配、三套真实 PDF 预览、导出质检和模拟面试。
...
该路径不要求 Docker、数据库或外部 AI。解析运行在 Next.js Node 进程中，使用 PDF.js 原生提取和离线 Tesseract.js 补充 OCR。
...
外部 AI 默认关闭。先撤销任何曾在聊天、终端输出或提交中暴露的旧 Key，再把轮换后的新 Key 写入被忽略的 `.env.local`：

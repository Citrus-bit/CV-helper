...
import { analyzeParsedResume, createCapabilityContext } from "@/lib/server/analysis";
import { jsonResponse, parseFormDataBodyLimited, RequestInputError, routeErrorResponse } from "@/lib/server/http";
import { parseWithDocumentWorker } from "@/lib/server/document-worker";
import { enforceAiRateLimit } from "@/lib/server/ai-rate-limit";
import { selectOcrBlocksForPage } from "@/lib/server/ocr-merge";
import { MAX_PDF_BYTES, type ParsedPdfResult, type ParsedSourceBlock } from "@/lib/server/pdf";
...
export async function POST(request: Request) {
  try {
    ...
    if (!(value instanceof File)) throw new RequestInputError("请选择一份 PDF 简历。");
    if (value.type && value.type !== "application/pdf" && !value.name.toLowerCase().endsWith(".pdf")) {
    ...
    const isolatedParse = await parseWithDocumentWorker({
      bytes,
      fileName: value.name || "resume.pdf",
      signal: request.signal,
    });
    const parseResult = isolatedParse ?? await invokeBaselineCapability(
      "document.parse",
      { pdfBase64: originalPdfBase64, fileName: value.name || "resume.pdf" },
      createCapabilityContext("mixed", ["original_pdf"], request.signal),
    );
    const documentVersions: Record<string, string> = { "document.parse": parseResult.sourceVersion };
    const combinedBlocks = [...parseResult.data.blocks];
    const ocrConfidence = new Map<number, number>();
    const ocrWarnings: string[] = [];
    const ocrContext = createCapabilityContext("mixed", ["page_image"], request.signal, 180_000);
    for (const page of parseResult.data.pages.filter(
      (candidate) => !isolatedParse && (candidate.source === "scan" || candidate.source === "mixed"),
    )) {
      try {
        ...
        const nativePageBlocks = parseResult.data.blocks.filter(
          (block) => block.pageIndex === page.pageIndex && block.source === "native",
        );
        combinedBlocks.push(...selectOcrBlocksForPage(page.source, nativePageBlocks, ocrResult.data.blocks));
        ocrConfidence.set(page.pageIndex, ocrResult.data.confidence);
        ocrWarnings.push(...ocrResult.warnings.map((warning) => warning.message));
      } catch {
        ocrWarnings.push(`第 ${page.pageIndex + 1} 页 OCR 暂时不可用，原始页面仍可预览。`);
      }
    }
    const sourceBlockContext = createCapabilityContext("mixed", ["source_blocks"], request.signal);
    ...
    const parsedBlocks: ParsedSourceBlock[] = segmentResult.data.blocks.map((block) => ({
      id: block.id,
    ...
    const hadNative = parsedBlocks.some((block) => block.source === "pdf" && block.text.trim());
    const hadOcr = parsedBlocks.some((block) => block.source === "ocr" && block.text.trim());

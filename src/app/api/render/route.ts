...
const EXPORT_GENERATION_TIMEOUT_MS = 120_000;
...
export async function POST(request: Request) {
  try {
    ...
    const context = createCapabilityContext(
      ...
      request.signal,
      EXPORT_GENERATION_TIMEOUT_MS,
    );
    const candidateInput = { ...input, template: FIXED_RESUME_TEMPLATE };
    ...
    const candidate = RenderResponseSchema.parse({
      ...
      generation: { attempts: 1, aiRepairApplied: false },
    });
    return jsonResponse(candidate, {
      headers: {
        "x-capability-trace": [rendered.sourceVersion, audited.sourceVersion].join(","),
      },
    });
  } catch (error) {

...
const NON_PERSON_CONTEXT_TERMS = /(?:客户|用户|团队|公司|部门|业务方|供应商|合作方|项目组|组织|机构|社区|平台|系统|产品|市场)$/u;
...
export class PiiProjector {
  ...
  assertSafe(value: unknown): void {
    ...
    if (
      this.containsSensitiveValue(serialized) ||
      patternMatches(POTENTIAL_LABELED_PII_PATTERN, serialized) ||
      patternMatches(POTENTIAL_CHINESE_ADDRESS_PATTERN, serialized) ||

...
export async function analyzeJobMatch(
  ...
  const redactionResult = await invokeBaselineCapability(
    "pii.redact",
    { text: input.jdText },
    securityContext,
  );
  ...
  // Metadata is independently guarded so short overrides cannot bypass the JD guard.
  const metadataEntries = await Promise.all(
    (["jobTitle", "seniority", "location"] as const).map(
      async (key) =>
        [key, await secureMetadataValue(input[key], securityContext)] as const,
    ),
  );
  ...
  const finalPosting = JobPostingSchema.parse({
    ...
    locale: capabilityLocale,
    // The original JD remains local for audit; providers receive guarded text.
  ...
  // A job variant may reorder evidence-backed facts, but never invent unmet claims.
  const variant = variantResult
    ...
    : undefined;
  const risks = riskResult.data.risks.map(
    (risk) => `${risk.explanation}（${risk.excerpt}）`,
  );
  if (guardResult.data.suspicious) {
    risks.unshift("岗位文本包含类似指令的内容，已按不可信数据处理。");
  }
  if (Object.values(metadata).some((result) => result?.suspicious)) {
    risks.unshift("岗位元信息包含类似指令的内容，已按不可信数据处理。");
  }
  return {
    bundle: JobMatchBundleSchema.parse({

...
import type { AnalysisBundle, EvaluationResponse } from "./contracts";
...
function analysisFixture(): AnalysisBundle {
  return {
    ...
    scorecard: {
      ...
      summary: "测试评分",
      dimensions: [
        { id: "impact", label: "成果影响", score: 10, maxScore: 20, evidence: [], deductions: [] },
        { id: "completeness", label: "完整性", score: 12, maxScore: 15, evidence: [], deductions: [] },
        { id: "clarity", label: "清晰度", score: 12, maxScore: 15, evidence: [], deductions: [] },
        { id: "structure", label: "结构", score: 12, maxScore: 15, evidence: [], deductions: [] },
        { id: "ats", label: "ATS", score: 12, maxScore: 15, evidence: [], deductions: [] },
        { id: "language", label: "语言", score: 12, maxScore: 20, evidence: [], deductions: [] },
      ],
    },
    suggestions: [
      {
...
function evaluation(questionId: string, overallScore: number): EvaluationResponse {
  return {
...
describe("evidence confirmation", () => {
  it("keeps the source claim, stages a precise rewrite, and applies it only after acceptance", () => {
    ...
    expect(accepted.resume.ast.sections[0].entries[0].bullets[0]).toBe(confirmedText);
    ...
    expect(accepted.evidence).toEqual(expect.arrayContaining([
      expect.objectContaining({ id: "resume-evidence-1", content: "建设发布流程并支持团队交付" }),
      expect.objectContaining({ id: "user-statement-claim-1", content: confirmedText, verifiedByUser: true }),
    ]));
  ...
  it("undoes both acceptance and the user-statement evidence without losing the original source", () => {
    useAppStore.getState().setAnalysis(analysisFixture());
    useAppStore.getState().confirmClaim("claim-1", "建设发布流程，将平均发布时间缩短 30%");
    useAppStore.getState().decideSuggestion("suggestion-1", "accepted");

    useAppStore.getState().undo();
    expect(useAppStore.getState().analysis).toMatchObject({
      resume: { revision: 0 },
      claims: [{ text: "建设发布流程并支持团队交付", status: "user_confirmed" }],
      suggestions: [{ kind: "rewrite", status: "pending" }],
      stories: [{ title: "建设发布流程并支持团队交付", result: "待补充：可核实的结果或影响" }],
    });
    ...
    expect(useAppStore.getState().analysis).toMatchObject({
      ...
      stories: [{ title: "建设发布流程并支持团队交付", evidenceAssetIds: ["resume-evidence-1"] }],
    });
    expect(useAppStore.getState().analysis!.suggestions[0]).not.toHaveProperty("proposedText");
  });
  ...
  it("treats a final manual rewrite as the verified user statement used by the claim and story", () => {
    ...
    useAppStore.getState().decideSuggestion("suggestion-1", "manual", finalText);

    const analysis = useAppStore.getState().analysis!;
    expect(analysis.resume.ast.sections[0].entries[0].bullets[0]).toBe(finalText);
    ...
    expect(analysis.evidence.find((asset) => asset.id === "user-statement-claim-1")).toMatchObject({
      content: finalText,
      verifiedByUser: true,
    });
  ...
  it("creates a story card when the confirmed claim did not have one", () => {
    ...
    expect(useAppStore.getState().analysis!.stories).toEqual([

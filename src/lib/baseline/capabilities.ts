...
function allResumeLines(
  input: EvidenceMineInput["resume"],
): Array<{ text: string; sourceBlockIds: string[] }> {
  const lines: Array<{ text: string; sourceBlockIds: string[] }> = [];
  if (input.ast.summary)
    lines.push({ text: input.ast.summary, sourceBlockIds: [] });
  ...
  (input) => {
    ...
    lines.forEach(({ text, sourceBlockIds }, index) => {
      ...
      const evidenceId = stableId(
        "evidence",
        `${input.resume.id}:${index}:${normalized}`,
      );
    ...
    return {
      ...
      evidenceReferences: [
        ...new Set(lines.flatMap((line) => line.sourceBlockIds)),
      ],
...
const claimAssessCapability = defineCapability(
  ...
  (input: ClaimAssessInput) => {
    const linked = input.evidenceAssets.filter((asset) =>
      input.claim.evidenceAssetIds.includes(asset.id),
    );
    ...
    const hasResumeSource =
      ...
      input.claim.sourceBlockIds.length > 0;
    const status = independentlySupported
      ...
      : userConfirmed
        ...
        : hasResumeSource
          ? "resume_only"
          : "needs_evidence";
    const confidence = independentlySupported
      ? 0.92
      : userConfirmed
        ? 0.82
        : hasResumeSource
...
const claimConflictCapability = defineCapability(
  ...
  (input: ClaimConflictInput) => {
    ...
    return {
      data: { conflicts },
      confidence: conflicts.length ? 0.7 : 0.55,
      evidenceReferences: input.claims.map((claim) => claim.id),
    };
  },
);
...
const resumeScoreCapability = defineCapability(
  ...
  (input: ResumeScoreInput) => {
    const entries = input.resume.ast.sections.flatMap(
    ...
    const dimensions = [
      dimension(
        ...
        8 +
          ...
          Math.min(8, input.claims.filter((claim) => claim.result).length),
        25,
  ...
  (input: ResumeSuggestInput) => {
    ...
    return {
      ...
      warnings: selectedSuggestions.length
        ...
        : [
            {
              code: "NO_RULE_FINDINGS",
              message: "内置规则未发现需要修改的明确问题。",
            },
          ],
    };
  },
...
const resumeAtsAuditCapability = defineCapability(
  ...
  (input: ResumeAtsAuditInput) => {
    ...
    const lowConfidence = input.resume.sourceBlocks.filter(
      (block) => block.confidence < 0.75,
    );
    const tables = input.resume.sourceBlocks.filter(
      (block) => block.role === "table",
    );
    ...
    if (tables.length)
      findings.push({
        code: "TABLE_LAYOUT",
        severity: "warning",
        message: "表格布局可能改变 ATS 阅读顺序。",
        sourceBlockIds: tables.map((block) => block.id),
      });
    if (lowConfidence.length)
      findings.push({
        ...
        severity: "warning",
        message: "部分文本解析置信度较低。",
        sourceBlockIds: lowConfidence.map((block) => block.id),
      });
    if (input.resume.ast.sections.length === 0)
      findings.push({
        code: "NO_SECTIONS",
        severity: "error",
        message: "未识别到标准简历板块。",
...
function classifyRequirement(line: string): {
  category: z.infer<
    typeof JdParseOutputSchema
  >["requirements"][number]["category"];
  importance: number;
} {
  if (/优先|加分|preferred|nice to have|bonus/i.test(line))
    return { category: "nice_to_have", importance: 0.45 };
  if (
    /必须|任职要求|本科|硕士|\d+\s*年|required|must|degree|years? of/i.test(
      line,
    )
  )
    return { category: "must_have", importance: 1 };
  if (/负责|职责|工作内容|you will|responsibilit|duties/i.test(line))
    return { category: "responsibility", importance: 0.8 };
  if (
    /地点|出差|薪资|到岗|location|travel|salary|on[- ]site|remote/i.test(line)
  )
    return { category: "constraint", importance: 0.8 };
...
const jdParseCapability = defineCapability(
  ...
  (input: JdParseInput) => {
    ...
    const inferredTitle =
      ...
      (input.locale === "en-US" ? "Target role" : "目标岗位");
    const jobId = stableId(
      "job",
      `${input.company ?? ""}:${inferredTitle}:${sourceText}`,
    );
...
const jobMatchCapability = defineCapability(
  ...
  (input: JobMatchInput) => {
    const maps = input.requirements.map((requirement) => {
      ...
      const best = ranked[0];
      const conflict = ranked.find(
        (item) => item.claim.status === "conflicting",
      );
      const status = conflict
        ? "conflict"
        : best &&
            best.overlap.length >=
              Math.min(2, Math.max(1, requirement.keywords.length))
          ? "met"
          : best
            ? "partial"
            : "gap";
      ...
      const evidenceAssetIds = [
        ...new Set(
          ranked.slice(0, 3).flatMap((item) => item.claim.evidenceAssetIds),
        ),
      ];
      return {
          ...
          status === "met"
            ...
            : status === "partial"
              ...
              : status === "conflict"
                ...
                : "当前简历中没有找到可追溯证据。",
        confidence:
          status === "gap" ? 0.62 : status === "partial" ? 0.65 : 0.75,
        suggestedAction:
          status === "gap"
    ...
    const coveredWeight = input.requirements.reduce(
      (sum, requirement, index) =>
        sum +
        requirement.importance *
          (maps[index].status === "met"
            ? 1
            : maps[index].status === "partial"
              ? 0.5
              : 0),
      0,
    );
    const evidenceCoverageRate = totalWeight
      ? round((coveredWeight / totalWeight) * 100, 1)
      : 0;
    return {
      data: {
        ...
        maps,
        disclaimer:
          "证据覆盖率仅表示简历材料与岗位要求的适配程度，不代表录取或获得面试的概率。",
      },
      confidence: input.requirements.length ? 0.7 : 0.35,
      evidenceReferences: [
        ...new Set(
          maps.flatMap((map) => [...map.claimIds, ...map.evidenceAssetIds]),
        ),
      ],
    };
  },
  ...
  (input: JobRiskDetectInput) => {
    const patterns: Array<{
    ...
    }> = [
      {
        code: "UPFRONT_FEE",
        severity: "high",
        pattern: /培训费|押金|保证金|先交费|registration fee|deposit/i,
        explanation: "正规招聘通常不会要求候选人先支付费用。",
      },
      {
        code: "DISCRIMINATORY",
        severity: "high",
        pattern:
          /仅限男性|仅限女性|已婚已育|年龄[：:]?\s*\d|male only|female only|marital status/i,
        explanation: "描述可能包含与岗位无关的歧视性限制。",
      },
      {
        code: "UNPAID_TRIAL",
        severity: "medium",
        pattern: /无薪试岗|免费试岗|unpaid trial/i,
        explanation: "无薪试岗可能带来劳动权益风险。",
      },
      {
        code: "VAGUE_COMPENSATION",
        severity: "low",
        pattern: /薪资面议|上不封顶|高额回报|competitive salary/i,
        explanation: "薪酬范围不明确，建议在沟通前确认结构和支付条件。",
      },
      {
        code: "CONTACT_OFF_PLATFORM",
        severity: "medium",
        pattern: /私人微信|个人账户|telegram|whatsapp/i,
        explanation: "要求转移至私人渠道时，应先核验公司和招聘方身份。",
      },
    ];
    ...
    return {
      data: { risks },
      confidence: risks.length ? 0.82 : 0.62,
      evidenceReferences: [input.jobPosting.id],
    };
  },
);

function rewriteCapability(
  id: "copy.rewrite.zh" | "copy.rewrite.en",
  locale: "zh" | "en",
) {
  return defineCapability(
    ...
    (input: CopyRewriteInput) => {
      ...
      const missingTerms = input.preserveTerms.filter(
        (term) => !rewritten.text.includes(term),
      );
      const finalText = missingTerms.length
...
const copyConsistencyCapability = defineCapability(
  ...
  (input: CopyConsistencyInput) => {
    const withTerminal = input.texts.filter((text) =>
    ...
    input.texts.forEach((text, index) => {
      ...
      if (hasTerminal !== useTerminal) {
        ...
        issues.push({
          ...
          suggestedText: useTerminal
            ? `${normalized.replace(/[。.!]$/, "")}${terminal}`
            : normalized.replace(/[。.!]$/, ""),
        });
      }
    });
    return {
      data: { consistent: issues.length === 0, issues },
      confidence: 0.9,
      evidenceReferences: [],
    };
  },
...
const BUILTIN_QUESTIONS: InterviewQuestion[] = [
  {
    id: "q_zh_star",
    locale: "zh-CN",
    prompt: "请讲一个你解决复杂问题的具体经历。",
    category: "behavioral",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["问题解决"],
    followUps: ["你个人采取了哪些行动？", "结果如何验证？"],
    scoringAnchors: ["情境清楚", "个人行动具体", "结果可核实"],
    source: "builtin-general-v1",
    generated: false,
  ...
  },
  {
    id: "q_zh_conflict",
    locale: "zh-CN",
    prompt: "请讲一次你与同事意见不一致并推动达成结果的经历。",
    category: "behavioral",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["协作", "沟通"],
    followUps: ["你如何理解对方的目标？", "事后会做什么不同选择？"],
    scoringAnchors: ["不推卸责任", "说明取舍", "有复盘"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_zh_project",
    locale: "zh-CN",
    prompt: "选择简历中的一个项目，说明你的职责、关键决策和最终结果。",
    category: "resume",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["项目管理"],
    followUps: ["哪个结果最能证明你的贡献？"],
    scoringAnchors: ["与简历一致", "区分个人和团队贡献"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_zh_failure",
    locale: "zh-CN",
    prompt: "讲一次结果没有达到预期的经历，你从中学到了什么？",
    category: "behavioral",
    difficulty: "advanced",
    roleFamilies: [],
    skills: ["复盘"],
    followUps: ["你后来如何验证改进有效？"],
    scoringAnchors: ["承认责任", "有具体改进"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_zh_role",
    locale: "zh-CN",
    prompt: "为什么这个岗位适合你当前的能力和下一步发展？",
    category: "motivation",
    difficulty: "introductory",
    roleFamilies: [],
    skills: ["岗位认知"],
    followUps: ["你仍需补强什么能力？"],
    scoringAnchors: ["结合岗位要求", "引用真实证据"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_zh_priority",
    locale: "zh-CN",
    prompt: "当多个重要任务同时临近截止时间时，你如何确定优先级？",
    category: "behavioral",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["优先级管理"],
    followUps: ["你如何同步风险？"],
    scoringAnchors: ["标准明确", "主动沟通"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_en_star",
    locale: "en-US",
    prompt: "Tell me about a time you solved a complex problem.",
    category: "behavioral",
    difficulty: "intermediate",
    roleFamilies: [],
  ...
  },
  {
    ...
    locale: "en-US",
    prompt:
      "Describe a disagreement with a colleague and how you reached an outcome.",
    category: "behavioral",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["collaboration", "communication"],
    followUps: ["How did you understand their goals?"],
    scoringAnchors: ["owns decisions", "explains trade-offs"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    ...
    locale: "en-US",
    prompt:
      "Choose one project from your resume and explain your role, key decisions, and result.",
    category: "resume",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["project management"],
    followUps: ["Which evidence best supports your contribution?"],
    scoringAnchors: [
      "consistent with resume",
      "separates team and individual work",
    ],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_en_failure",
    locale: "en-US",
    prompt: "Tell me about an outcome that fell short and what you learned.",
    category: "behavioral",
    difficulty: "advanced",
    roleFamilies: [],
    skills: ["reflection"],
    followUps: ["How did you validate your improvement?"],
    scoringAnchors: ["owns responsibility", "specific change"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    ...
    locale: "en-US",
    prompt:
      "Why is this role a good fit for your current strengths and next step?",
    category: "motivation",
    difficulty: "introductory",
    roleFamilies: [],
    skills: ["role understanding"],
    followUps: ["What capability do you still need to build?"],
    scoringAnchors: ["connects to role", "uses real evidence"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
  {
    id: "q_en_priority",
    locale: "en-US",
    prompt: "How do you prioritize when several important deadlines collide?",
    category: "behavioral",
    difficulty: "intermediate",
    roleFamilies: [],
    skills: ["prioritization"],
    followUps: ["How do you communicate risk?"],
    scoringAnchors: ["clear criteria", "proactive communication"],
    source: "builtin-general-v1",
    generated: false,
    referenceQuestionIds: [],
  },
...
const questionRetrieveCapability = defineCapability(
  ...
  (input: QuestionRetrieveInput) => {
    ...
    const ranked = catalog
      ...
      .map((question, index) => ({
        ...
        score: keywordOverlap(
          [...question.skills, ...question.roleFamilies],
          requestedKeywords,
        ).length,
      }))
    ...
    return {
      data: { questions: ranked },
      confidence: input.catalog.length ? 0.78 : 0.65,
      evidenceReferences: ranked.map((question) => question.id),
    };
  },
);
...
const interviewPlanCapability = defineCapability(
  ...
  (input: InterviewPlanInput) => {
    ...
    const targetMinutes = selected.length
      ? round(input.durationMinutes / selected.length, 1)
    ...
    return {
      ...
      warnings:
        selected.length < input.questionCount
          ? [
              {
                code: "QUESTION_SHORTAGE",
                message: "题库数量不足，计划已按可用题目缩短。",
              },
            ]
          : [],
    };
  ...
  (input: StoryBuildInput) => {
    ...
    return {
      data: {
        ...
        riskNotes: [
          ...(input.claim.status === "needs_evidence"
            ? ["当前声明缺少证据，练习时不要补造细节。"]
            : []),
          ...(!input.claim.result
            ? ["结果尚不完整，回答前应补充真实信息。"]
...
const answerEvaluateCapability = defineCapability(
  ...
  (input: AnswerEvaluateInput) => {
    ...
    const targetKeywords = [
      ...input.question.skills,
      ...input.expectedKeywords,
    ];
    const overlap = keywordOverlap(answer, targetKeywords);
    const relevance = clamp(
      7 + overlap.length * 4 + (length >= 20 ? 3 : 0),
      0,
      20,
    );
    const structureMarkers = [
      /(?:当时|背景|situation|context)/i,
      /(?:目标|任务|task|goal)/i,
      /(?:我|本人|I\s)/i,
      /(?:结果|最终|提升|降低|result|outcome)/i,
    ];
    const structure = clamp(
      5 +
        structureMarkers.filter((pattern) => pattern.test(answer)).length * 3.5,
      0,
      20,
    );
    const evidence = clamp(
      4 +
        numericTokens(answer).length * 4 +
        (/(?:例如|具体|because|for example|specifically)/i.test(answer)
          ? 4
          : 0),
      0,
      20,
    );
    const roleCompetency = clamp(
      7 +
        overlap.length * 4 +
        (/(?:权衡|决策|协作|分析|trade-?off|decision|collaborat|analy)/i.test(
          answer,
        )
          ? 3
          : 0),
    ...
    const fragments = splitStatements(answer)
      .filter(
        (statement) =>
          numericTokens(statement).length ||
          keywordOverlap(statement, targetKeywords).length,
      )
      .slice(0, 3);
...
const answerCoachCapability = defineCapability(
  ...
  (input: AnswerCoachInput) => {
    const weakest = Object.entries(input.evaluation.dimensions)
      .sort((a, b) => a[1] - b[1])
      .slice(0, 2)
      .map(([name]) => name);
    const actionByDimension: Record<string, string> = {
      ...
      evidence:
        "从真实经历中补充一个可核实结果；没有数字时可说明范围、反馈或交付物。",
    ...
    return {
      data: {
        headline:
          input.evaluation.overallScore >= 80
            ? "回答基础扎实，继续强化证据。"
            : "先收紧结构，再补充真实证据。",
        ...
        factSafetyReminder:
          "只使用你能在追问中解释并核实的信息，不补造数字或扩大个人贡献。",
      },
      confidence: 0.86,
      evidenceReferences: [input.question.id],
    };
...
const resumeInterviewCheckCapability = defineCapability(
  ...
  (input: ResumeInterviewCheckInput) => {
    ...
    const findings: z.infer<
      typeof ResumeInterviewCheckOutputSchema
    >["findings"] = [];
    for (const claim of input.claims) {
      const overlap = keywordOverlap(input.answer, claim.text);
      if (!overlap.length) continue;
      const claimNumbers = numericTokens(claim.text);
      const unmatchedAnswerNumbers = answerNumbers.filter(
        (number) => !claimNumbers.includes(number),
      );
      if (claimNumbers.length && unmatchedAnswerNumbers.length) {
        findings.push({
          claimId: claim.id,
          severity: "warning",
          answerExcerpt: excerpt(input.answer),
          resumeExcerpt: excerpt(claim.text),
          explanation: `回答出现简历中没有的数值（${unmatchedAnswerNumbers.join("、")}），请核对口径。`,
        });
      }
      if (claim.status === "conflicting" || claim.status === "needs_evidence") {
        findings.push({
          ...
          answerExcerpt: excerpt(input.answer),
          resumeExcerpt: excerpt(claim.text),
          explanation: "关联简历声明本身仍待核对，请勿在回答中进一步扩大。",
        });
      }
    }
    return {
      data: { consistent: findings.length === 0, findings },
      confidence: findings.length ? 0.72 : 0.55,
      evidenceReferences: findings.flatMap((finding) =>
        finding.claimId ? [finding.claimId] : [],
      ),
    };
  },
);
...
const piiRedactCapability = defineCapability(
  ...
  (input: PiiRedactInput) => {
    ...
    const rules: Array<{
      type: "email" | "phone" | "id_number" | "address" | "url";
      pattern: RegExp;
      placeholder: string;
    }> = [
      {
      ...
      },
      {
        type: "phone",
        pattern:
          /(?<!\d)(?:\+?86[-\s]?)?1[3-9]\d{9}(?!\d)|(?<!\d)(?:\+?\d{1,3}[-\s]?)?(?:\d[-\s]?){7,12}(?!\d)/g,
        placeholder: "[PHONE]",
      },
      {
        type: "address",
        pattern: /(?:地址|住址|Address)\s*[：:]\s*[^\n,，;；]{6,80}/gi,
        placeholder: "[ADDRESS]",
      },
      {
        type: "url",
    ...
    for (const rule of rules) {
      if (rule.pattern.test(redactedText)) {
        ...
        for (let index = 0; index < count; index += 1)
          detections.push({ type: rule.type, placeholder: rule.placeholder });
      }
    }
    return {
      data: { redactedText, detections },
      confidence: 0.88,
      evidenceReferences: [],
      warnings: [
        {
          code: "REVIEW_REDACTION",
          message: "规则脱敏可能漏掉非标准格式，请在发送给外部服务前复核。",
        },
      ],
    };
  },
  ...
  (input: PromptGuardInput) => {
    const patterns: Array<{ code: string; pattern: RegExp }> = [
      {
        ...
        pattern:
          /ignore (?:all |any )?(?:previous|prior|system) instructions?|忽略(?:之前|以上|系统)(?:的)?(?:指令|要求)/gi,
      },
      {
        code: "ROLE_OVERRIDE",
        pattern: /you are now|act as (?:a|an)|你现在是|扮演(?:一个)?/gi,
      },
      {
        code: "SECRET_REQUEST",
        pattern:
          /system prompt|developer message|api key|系统提示词|开发者消息|密钥/gi,
      },
      {
        code: "TOOL_INSTRUCTION",
        pattern:
          /run (?:this )?(?:command|tool)|execute (?:a )?command|运行(?:这个)?命令|调用(?:这个)?工具/gi,
      },
    ];
    const findings = patterns.flatMap((item) => {
      ...
      return matches.map((match) => ({
        code: item.code,
        excerpt: excerpt(match[0], 80),
        action: "treat_as_untrusted_data" as const,
      }));
    });
    const safeText = wrapUntrustedDocumentText(input.text);
    return {
      data: { safeText, suspicious: findings.length > 0, findings },
      confidence: findings.length ? 0.9 : 0.7,
      evidenceReferences: [],
    };
  },
);
